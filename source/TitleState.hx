...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var spaceTex = Paths.getSparrowAtlas('animatedbg');

		Hearts = new FlxSprite( -160, -210);
		Hearts.frames = spaceTex;
		Hearts.animation.addByPrefix('play', 'Blue animation bg', 24, true);
		Hearts.animation.play('play');
		Hearts.setGraphicSize(Std.int(Hearts.width * 1.5));
		Hearts.antialiasing = true;
		add(Hearts);

		var purp:BGSprite = new BGSprite('Purple', -650, -100, 0.9, 0.9);
		purp.setGraphicSize(Std.int(purp.width * 1.3));
		purp.updateHitbox();
		add(purp);

		var stageFront:BGSprite = new BGSprite('Speech', 250, 0, 0.9, 0.9);
